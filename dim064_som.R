...
dim064_test_label <- read.csv('dim064_test_label.csv')

######## Original Data #########
dim064_test <- read.csv('dim064_test.csv')
...
ms <- map.build(dim064_test, labels = dim064_test_label, xdim = 40, ydim = 35,alpha=0.6,
                train =10000,algorithm="vsom")
ms1 <- map.build(dim064_test, labels = dim064_test_label, xdim = 20, ydim = 15,alpha=0.6,
...
########## Basic AE ############
dim064_AE <- read.csv('dim064_AE_encoded.csv')
